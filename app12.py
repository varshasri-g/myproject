...
def a12():
    ...
    sheet_name ='12.Finance'  # Adjust if necessary
    ...
    df['Category'] = df['Process'].fillna(method='ffill')

    # Drop rows where 'Program Discovery Questions' column is NaN
    questions_df = df.dropna(subset=['Scope Question'])

    # Group questions by 'Category' and convert them to a dictionary
    questions_by_category = {
        category: questions['Scope Question'].tolist()
    ...
    # Streamlit app setup
    st.title("Finance Questionnaire")

    # Initialize session state variables
    if 'user_answers12' not in st.session_state:
        st.session_state.user_answers12 = {category: [""] * len(questions) for category, questions in questions_by_category.items()}

    if 'suggestions12' not in st.session_state:
        st.session_state.suggestions12 = {category: [""] * len(questions) for category, questions in questions_by_category.items()}
    ...
    def get_suggestion(question):
        try:
            ...
            response = palm.generate_text(
                ...
                prompt=f"Provide a suggestion for the following question according to the finance: {question}",
    ...
    if selected_category and selected_category != "Summary":
        ...
        for i, question in enumerate(questions):
            ...
            user_answer = st.text_input("Your answer", value=st.session_state.user_answers12[category][i], key=f"answer_input_{category}_{i}")
            st.session_state.user_answers12[category][i] = user_answer
            ...
            if st.session_state.suggestions12[category][i]:
                st.write("Suggestion:")
                st.text_area("Suggested Answer", value=st.session_state.suggestions12[category][i], height=100, key=f"suggestion_text_{category}_{i}")
    ...
    if selected_category == "Summary":
        ...
        for category, answers in st.session_state.user_answers12.items():
        ...
        if st.button("Submit"):
            try:
                ...
                if summary_df.shape[0] > 0:
                    ...
                    file_path = f"C:/Users/Guest_User/Desktop/Database/finance/{file_name}"  
                    ...
                    word_file_path = f"C:/Users/Guest_User/Desktop/Database/finance/Scopes/{word_file_name}"
                    doc = Document()
