...
def a11():
    ...
    sheet_name ='11.Plant Maintenance'  # Adjust if necessary
    ...
    # Streamlit app setup
    st.title("Plant Maintenance Questionnaire")

    # Initialize session state variables
    if 'user_answers11' not in st.session_state:
        st.session_state.user_answers11 = {category: [""] * len(questions) for category, questions in questions_by_category.items()}

    if 'suggestions11' not in st.session_state:
        st.session_state.suggestions11 = {category: [""] * len(questions) for category, questions in questions_by_category.items()}
    ...
    def get_suggestion(question):
        try:
            ...
            response = palm.generate_text(
                ...
                prompt=f"Provide a suggestion for the following question according to the plant maintenance: {question}",
    ...
    if selected_category and selected_category != "Summary":
        ...
        for i, question in enumerate(questions):
            ...
            user_answer = st.text_input("Your answer", value=st.session_state.user_answers11[category][i], key=f"answer_input_{category}_{i}")
            st.session_state.user_answers11[category][i] = user_answer
            
            if st.button("Suggestion", key=f"suggestion_{category}_{i}"):
                suggestion = get_suggestion(question)
                st.session_state.suggestions11[category][i] = suggestion
                st.experimental_rerun()
            
            # Display the suggestion if it exists
            if st.session_state.suggestions11[category][i]:
                st.write("Suggestion:")
                st.text_area("Suggested Answer", value=st.session_state.suggestions11[category][i], height=100, key=f"suggestion_text_{category}_{i}")
    ...
    if selected_category == "Summary":
        ...
        for category, answers in st.session_state.user_answers11.items():
        ...
        if st.button("Submit"):
                try:
                    ...
                    if summary_df.shape[0] > 0:
                        ...
                        file_path = f"C:/Users/Guest_User/Desktop/Database/plant maintenance/{file_name}"  
                        ...
                        word_file_path = f"C:/Users/Guest_User/Desktop/Database/plant maintenance/scopes/{word_file_name}"
                        doc = Document()
