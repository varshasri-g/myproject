...
def a3():
    ...
    if 'user_answers2' not in st.session_state:
        st.session_state.user_answers2 = {category: [""] * len(questions) for category, questions in questions_by_category.items()}

    if 'suggestions2' not in st.session_state:
        st.session_state.suggestions2 = {category: [""] * len(questions) for category, questions in questions_by_category.items()}
    ...
    if selected_category and selected_category != "Summary":
        ...
        for i, question in enumerate(questions):
            ...
            user_answer = st.text_input("Your answer", value=st.session_state.user_answers2[category][i], key=f"answer_input_{category}_{i}")
            st.session_state.user_answers2[category][i] = user_answer
            ...
            if st.session_state.suggestions2[category][i]:
                st.write("Suggestion:")
                st.text_area("Suggested Answer", value=st.session_state.suggestions2[category][i], height=100, key=f"suggestion_text_{category}_{i}")
    ...
    if selected_category == "Summary":
        ...
        for category, answers in st.session_state.user_answers2.items():
